...
elif [ "$1" = "dev_train_cuda" ]; then
	CUDA_VISIBLE_DEVICES=0 python run.py train --train-src=./en_es_data/train_small.es --train-tgt=./en_es_data/train_small.en --dev-src=./en_es_data/train_small.es --dev-tgt=./en_es_data/train_small.en --vocab=vocab_small.json --cuda
elif [ "$1" = "dev_test_cuda" ]; then
	echo "Performance of untrained model from CUDA"
    CUDA_VISIBLE_DEVICES=0 python run.py decode model_benchmark.bin ./en_es_data/train_small.es ./en_es_data/train_small.en ./outputs/dev_outputs.txt --cuda
	echo "Performance of trained model from CUDA"
    CUDA_VISIBLE_DEVICES=0 python run.py decode model.bin ./en_es_data/train_small.es ./en_es_data/train_small.en ./outputs/dev_outputs.txt --cuda
else
